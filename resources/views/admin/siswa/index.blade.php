...
<div class="container-fluid">
    ...
    <ul class="nav nav-pills nav-fill fw-bold mt-4 shadow-sm p-2 bg-light rounded" id="tabSiswa" role="tablist">
        ...
        <li class="nav-item">
            <button class="nav-link {{ $index == 0 ? 'active' : '' }} py-2 text-uppercase shadow-sm" id="tab-{{ $tkt }}" data-bs-toggle="tab" data-bs-target="#pane-{{ $tkt }}" type="button">
                📁 KELAS TINGKAT {{ $tkt }}
            </button>
        </li>
        @endforeach
            ...
            <div class="accordion" id="accordion-{{ $tkt }}">
                ...
                <div class="accordion-item mb-2 border shadow-sm">
                    <h2 class="accordion-header">
                        <button class="accordion-button collapsed fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $idKelasSafe }}">
                            📂 {{ $namaKelas }} &nbsp;
                            <span class="badge bg-primary ms-2">{{ $daftarSiswa->count() }} Siswa</span>
                        </button>
                    </h2>
                    <div id="collapse-{{ $idKelasSafe }}" class="accordion-collapse collapse" data-bs-parent="#accordion-{{ $tkt }}">
                            ...
                            <table class="table table-bordered table-striped mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $no + 1 }}</td>
                                        <td>
                                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=50x50&data={{ $siswa->nis }}" alt="QR">
                                        </td>
                                        <td>{{ $siswa->nis }}</td>
                                        <td class="fw-bold text-uppercase">{{ $siswa->nama_siswa }}</td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <a href="{{ route('siswa.cetak-qr', $siswa->id) }}" target="_blank" class="btn btn-sm btn-dark">🖨️ Cetak QR</a>
                ...
                <form action="{{ route('siswa.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama_siswa" class="form-control" required placeholder="Masukkan Nama Lengkap">
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="kelas" class="form-select" required>
                                ...
                                <optgroup label="Tata Busana (TBS)">
